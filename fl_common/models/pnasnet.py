...
def get_pnasnet_model(pnasnet_type, num_classes):
    ...
    if pnasnet_type not in valid_types:
        msg = f"Unknown Pnasnet Architecture: {pnasnet_type}"
        raise ValueError(msg)

    # Attempt to create the model with pretrained weights
    try:
        return create_model(
            pnasnet_type, pretrained=True, num_classes=num_classes,
        )
    except RuntimeError as e:
        print(f"{pnasnet_type} - Error loading pretrained model: {e}")
        return create_model(
            pnasnet_type, pretrained=False, num_classes=num_classes,
        )
