...
if torch.cuda.is_available():
    ...
    for i in range(torch.cuda.device_count()):
        ...
        print(f" - [{i}] {name} | capability={cap[0]}.{cap[1]} | VRAM={mem_gb:.2f} GB")
...
if mps_backend is None:
    ...
    print(f"MPS available: {_safe('mps_avail', mps_backend.is_available, False)}")
...
if xpu is None:
    ...
    if _safe('xpu_avail', xpu.is_available, False) is True:
        ...
        try:
            n = xpu.device_count()
            for i in range(n):
                name = _safe("xpu_name", lambda: xpu.get_device_name(i), "Unknown")
                print(f" - [{i}] {name}")
        except Exception as e:
            print(f" - Unable to enumerate XPU devices ({type(e).__name__}: {e})")
