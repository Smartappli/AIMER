...
async def module_all_details():
    from timm import list_models, list_modules
    return {module: list(list_models(module=module))
            for module in list_modules()}


mcp = FastMCP.from_fastapi(app=api, name="Timm API MCP")  # auto-tooling :contentReference[oaicite:1]{index=1}
mcp_app = mcp.http_app(path="/mcp")  # endpoint MCP :contentReference[oaicite:2]{index=2}
